(function executeRule(current, previous /*null when async*/) {
	...
	if (gs.getProperty("x_cir.SharedFolders.start") != "true") {
        ...
        this.addErrorLog('ERROR', "Integration Create Shared Folder triggered but it is disabled", 'CREATE_SHARED_FOLDER', 'OUT', '', current.customer.sys_id, current.number);
		return;
	}
	...
	if(current.share_folder_project == true && current.share_folder_orgswit == true && current.share_folder_orgswit_src == true){
        ...
        this.addErrorLog('INFO', "Shared Folders were already created. You cannot create same folders twice", 'CREATE_SHARED_FOLDER', 'OUT', '', current.customer.sys_id, current.number);
		return;
	}


	var pathToScript = gs.getProperty('x_cir.SharedFolders.pathToScript');
	var mid_name = gs.getProperty('x_cir.SharedFolders.MID');
	var MID  = "mid.server." + mid_name;
	...
	var path_customerOU = current.customer.customer_number.toUpperCase() + "_" + (current.customer.organizational_unit.replace(/\s/gm, "_")).toUpperCase();
	...
	if(current.share_folder_orgswit == false){
		...
		reply_orgswit = true;
		// the check if response has error is further down

	} else {
        if(gs.getProperty("x_cir.SharedFolders.logExisting") == "true")
            addErrorLog('INFO', "Project folder is already existing. Cancelling the create project share folder request", 'CREATE_SHARED_FOLDER', 'OUT', '', current.customer.sys_id, current.number);
        reply_orgswit = false;
	}
	...
	if(current.share_folder_project == false){
        ...
        if(gs.getProperty("x_cir.SharedFolders.logExisting") == "true")
            this.addErrorLog('INFO', "Orgswit folder is already existing. Cancelling the create orgswt share folder request", 'CREATE_SHARED_FOLDER', 'OUT', '', current.customer.sys_id, current.number);
        reply_project = false;
    }
    ...
    if(current.share_folder_orgswit_src == false){
        ...
        if(gs.getProperty("x_cir.SharedFolders.logExisting") == "true")
            this.addErrorLog('INFO', "Orgwit Sources folder is already existing. Cancelling the create project share folder request", 'CREATE_SHARED_FOLDER', 'OUT', '', current.customer.sys_id, current.number);
	...
	projectErr = (errRegex.test(response_project.output) == true) ? true : false;
    orgswitErr = (errRegex.test(response_orgswit.output) == true) ? true : false;
    orgswitSRCErr = (errRegex.test(response_orgswit_src.output) == true) ? true : false;
	...
	if(reply_project){
		...
		if(projectErr){
            ...
            this.addErrorLog('ERROR', "Something went wrong creating Project Folder. Error message :\n " + response_project.output, 'CREATE_SHARED_FOLDER', 'OUT', mid_name, current.customer.sys_id, current.number);
	...
	if(reply_orgswit){
		...
		if(orgswitErr){
            ...
            this.addErrorLog('ERROR', "Something went wrong creating Orgswit Folder. Error message :\n " + response_orgswit.output, 'CREATE_SHARED_FOLDER', 'OUT', mid_name, current.customer.sys_id, current.number);
    ...
    if(reply_orgswit_src){
		...
		if(orgswitSRCErr){
            ...
            this.addErrorLog('ERROR', "Something went wrong creating Orgswit Source Folder. Error message :\n " + response_orgswit_src.output, 'CREATE_SHARED_FOLDER', 'OUT', mid_name, current.customer.sys_id, current.number);
