...
var caller = g_form.getReference('caller_id', checkVIP);

function checkVIP(caller) {
	if (caller.vip == 'true'){
		return true;
	} else {
		return false;
...
function prepareCallProbePkg()
{

	
alert("Sarting sync: waiting process time less then 2 mins for answer");
var ga = new GlideAjax('global.PowershellClientCall_Lib');
ga.addParam('sysparm_name', 'callPSPackageSynch');
ga.addParam('sysparm_sysid', g_form.getUniqueValue());
ga.addParam('sysparm_synchall', false);
ga.addParam('sysparm_manual', true);
ga.getXML(callPowerShellPkg);
 
}
function callPowerShellPkg(response) {
...
script += payload + ' -WebServiceEndpoint \" ' + mids.automation_server + '\" -RunbookName \"Remove-SNApplication\" -ReturnOutputStream -TimeoutSeconds 900 -Quiet | Select-Object -ExpandProperty \"StreamText\"}';
...
Invoke-SCWSMARunbook -WebServiceEndpoint [ https://sc001408.testwms.ch|https://sc001408.testwms.ch/] -RunbookName "Global" -ReturnOutputStream -Quiet | Select -ExpandProperty StreamText
