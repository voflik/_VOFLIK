function($http, $scope, i18n, nowAttachmentHandler, $timeout, $location) {
	...
	c.showSignatureMessage = function() {
		c.showInlineErrorMessage('${You must complete the required signature}');
	};

	c.showMandatoryMessage = function() {
		c.showInlineErrorMessage('${All mandatory fields must be filled before submission}');
	};

	c.showInvalidResponseMessage = function() {
		c.showInlineErrorMessage('${Please fill all the fields with valid responses}');
	};
	...
	if (!c.data.questions || !c.data.questions.idList || !c.data.questions.idList.length) {
		c.showInlineErrorMessage('${Invalid survey}');
		c.invalidSurvey = true;
		return;
	}

	if (c.data.state == 'complete' && !c.data.can_retake) {
		c.showInlineErrorMessage('${This survey has already been completed}');
		c.invalidSurvey = true;
	}
	...
	c.submitAjax = function(formdata) {
		...
		function successHandler(responseData) {
			if (responseData.success && (responseData.success || responseData.success == 'true')) {
				c.data.conclusion = (c.data.conclusion && c.data.conclusion.length) ? c.data.conclusion : '${Your responses have been submitted, thanks for taking the survey!}';
				...
				if (formdata.sysparm_action == 'submit' || formdata.updateSuccessMessage)
					c.data.successMessage = (formdata.sysparm_action == 'submit') ? c.data.conclusion :
												('${Your responses have been saved. You can complete this survey before:}' + c.data.instanceDuedate);
			} else {
				c.showInlineErrorMessage('${Something went wrong. Please try again later.}');
				c.invalidSurvey = true;
			}
