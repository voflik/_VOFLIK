...
Paysafe_SDCaller_Utils.prototype = {
 	...
 	*/
	sigOne: /^(?:Paysafe\sGroup\splc){1,}?(.*(\w|\W)){1,}?(.*transmission.){1,}|<(?:.|\n)*?>/gmi,
		/*
		@sigTwo property
		@string

 		*/
		sigTwo: /(Please.*print\.)/gmi,
			...
			cleanUpEmailSubject: function(subject) {
				...
				try {
					...
					for(var i = 0; i < prefixes.length; i++) {
						cleanSubject = subject.replace(prefixes[i],'');
					}
					cleanSubject = cleanSubject.replace(/([r]|[a]|[f]|[w]|[о]|[а]|[\S]).{1,2}\.?\:\s?/gmi, "").replace(/^\s{1}|\s{1,3}$/gmi, "");
					this.log('cleanUpEmailSubject: cleaned up ' + cleanSubject);
					...
					@param String
					@returns String
					// Fix - new RegExp for signature :
					aded new variables (^Paysafe\sGroup\splc.+(\n|\r)+.*(?=transmission\.).*)

					Used to replace body from signature via RegEx with empty string
 					*/
					cleanUpSignature: function(body){
						body = body.replace(this.sigOne, "").replace(this.sigTwo, "").replace(this.sigThree, "").replace(this.sigFour, "");
						this.log("cleanUpSignature: cleaned up \n " + body);
						return body;

					},
					/*
					@checkInetranlCompany method
					@param String
					@returns String

					Used to replace body from signature via RegEx with empty string
 					*/
					checkInetrnalCompany: function(mailTo){
						//TODO : Hardcoded values to be replaced by dynamic values!!! NTL,SKR,NBX,etc.
						//add record from company table. This demands new method
						//defaults to Paysafe Group
						var regEx = /((psc|ntl|skr|nbx|mrt)\-ops@paysafe.com)/gmi;
						this.log("checkInetrnalCompany, regex is : " + regEx + "  ,mailTo is : " + mailTo);


						var test = mailTo.replace(regEx, '$2').toLowerCase();
						switch(test.toString()) {
							case "skr" :
							this.log("checkInetrnalCompany  RES resolved to SKR " + test);
							return "Skrill";
							case "ntl" :
							this.log("checkInetrnalCompany  RES resoved to NTL " + test);
							return "Neteller";
							case "nbx" :
							this.log("checkInetrnalCompany  RES resolved to NBX " + test);
							return "Netbanx (NETBANX)";
							case "psc" :
							this.log("checkInetrnalCompany  RES resolved to PSC " + test);
							return "Paysafecard";
							case "mrt" :
							this.log("checkInetrnalCompany  RES resolved to MRT " + test);
							return "Meritus";
							default :
							this.log("checkInetrnalCompany RES resolved to DEFAULT = Paysafe   " + test);
							return "Paysafe Group";
					...
					@param String
					@returns void
cd72ae70db4e0bc05da998f3db961991
					...
					addCallToRelationship: function (sdcall, subject) {
						...
						existingRecord.u_active = true;
						existingRecord.u_sdcall = sdcall;
						existingRecord.u_subject = subject;
						existingRecord.u_mail_subject = email.subject;
